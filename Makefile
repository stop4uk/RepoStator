...
ifeq ($(INSIDE_DOCKER_CONTAINER), 0)
	@HOST_UID=$(HOST_UID) HOST_GID=$(HOST_GID) WEB_PORT_HTTP=$(WEB_PORT_HTTP) WEB_PORT_SSL=$(WEB_PORT_SSL) XDEBUG_CONFIG=$(XDEBUG_CONFIG) INNODB_USE_NATIVE_AIO=$(INNODB_USE_NATIVE_AIO) docker compose $(PROJECT_NAME) exec $(OPTION_T) $(PHP_USER) php-fpm bash
else
	$(ERROR_ONLY_FOR_HOST)
endif
...
ifeq ($(INSIDE_DOCKER_CONTAINER), 1)
	...
	@HOST_UID=$(HOST_UID) HOST_GID=$(HOST_GID) WEB_PORT_HTTP=$(WEB_PORT_HTTP) WEB_PORT_SSL=$(WEB_PORT_SSL) XDEBUG_CONFIG=$(XDEBUG_CONFIG) INNODB_USE_NATIVE_AIO=$(INNODB_USE_NATIVE_AIO) docker compose $(PROJECT_NAME) exec $(OPTION_T) $(PHP_USER) php-fpm $$cmd
endif
